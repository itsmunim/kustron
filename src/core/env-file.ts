...
function validatePortPlaceholders(envFile: EnvFile): void {
  for (const app of envFile.apps) {
    if (app.port === 'PORT') {
      throw new Error(`Set the port for '${app.name}' in kustron-env.yaml before running.`)
    }
  }
}
...
export async function parseEnvFileContent(content: string): Promise<EnvFile> {
  ...
  const envFile: EnvFile = {
    ...
    apps: result.apps.map((app) => ({
      ...app,
      port: app.port === 'PORT' ? 'PORT' : typeof app.port === 'string' ? parseInt(app.port, 10) || app.port : app.port,
    })),
  }
...
export async function appendApp(filePath: string, entry: AppEntry): Promise<void> {
  const content = await readFile(filePath, 'utf-8')
  const parsed = load(content) as { config?: { namespace?: string }; apps?: unknown[] }
...
export async function createEnvFile(filePath: string, entry: AppEntry): Promise<void> {
  const data: EnvFile = {
...
export function renderSpec(): string {
  return chalk.cyan(`# kustron-env.yaml Schema Reference
