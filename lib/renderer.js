...
const imagesTplDirPath = path.resolve(__dirname, '../templates/images');
const makefilesTplDirPath = path.resolve(__dirname, '../templates/makefiles');
const pipelinefilesTplDirPath = path.resolve(__dirname, '../templates/pipelines');
...
function renderImageFile(data) {
  const imgFileName = `${data.app.language}dockerfile`;
  return render(path.join(imagesTplDirPath, imgFileName), data);
}
